...
class Produk_model extends CI_Model {
	...
	public function create_diskon(){
		...
		$this->db->insert('discount_cashier2',$data);	
	...
	public function loadDiscount($nama,$first_date,$second_date) {
		...
		$query = $this->db->get('discount_cashier2');
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
			return 0;
    ...
    $this->db->from('transaksi2 t');
    $this->db->join('kapster2 k','t.id_kapster = k.id_kapster', 'left outer');
    ...
    $this->db->from('transaksi2 t');
    $this->db->join('cashier2 c','t.id_cashier = c.id_cashier', 'left outer');
    $this->db->join('produk p','t.id_produk = p.id_produk', 'left');
