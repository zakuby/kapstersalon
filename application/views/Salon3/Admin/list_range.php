...
<script>
        ...
        function pilih_produk(pilihan) {
			...
			}else if(pilihan=="Range"){
				document.getElementById("tujuan").innerHTML = "";
				document.formlist.action = "<?php echo base_url(); ?>Salon3/Admin/list_range";
			}else if(pilihan=="Cashier"){
				document.getElementById("tujuan").innerHTML = "";
				document.formlist.action = "<?php echo base_url(); ?>Salon3/Admin/list_cashier";
			}
        };
		...
		<div class="form-group">
            <label for="sel1">Product</label> <br/>
				<select class="form-control" name="pilihan" id="asal" onchange="pilih_produk(this.value);" required>
					<option value="Range">Range Gaji</option>
...
<table id="example2" class="table table-bordered table-hover">
            ...
            foreach ($g as $gaji) {
                print "<tr>";
                print "<td>";
                echo 'Rp. ' . number_format( $gaji->range_bawah, 0 , '' , '.' ) ;
                print "<td>";
                echo 'Rp. ' . number_format( $gaji->range_atas, 0 , '' , '.' ) ;
                print "<td>";
				print  $gaji->rate_gaji.'%' ; 
                echo '<td align="center">';
				echo "<a href='".site_url('Salon3/Admin/editRange/'.$gaji->id_range)."')'><button>Edit</button></a> ";
				echo "<a onClick=\"javascript: return confirm('Are you sure to delete ?');\" href='".site_url('Salon3/Admin/hapusRange/'.$gaji->id_range)."')'><button>X</button></a>";
            }
            ?>
			                </tfoot>
              </table>
